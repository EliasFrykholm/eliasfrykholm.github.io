...
function Navbar({ coloredComponents, offsetY }: Props) {
  const [fillColor, setFillColor] = useState(generateHslString(generateColor(0)));

  useEffect(() => {
    const index = coloredComponents?.findIndex((elem) => elem.getBoundingClientRect().y > 0);
    index && setFillColor(generateHslString(generateColor(index)));
  }, [coloredComponents, offsetY]);
