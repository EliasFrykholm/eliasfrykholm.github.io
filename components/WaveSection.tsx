...
function WaveSection({ isBottom = false, scrollOffset, index, children }: Props) {
  ...
  return (
    <div>
      ...
      {isBottom ? (
        <Wave isBottom={isBottom} scrollOffset={scrollOffset} color={color} />
      ) : (
        <div className={styles.infillContainer}>
          <div className={styles.bottomInfill} style={{ backgroundColor: color }} />
        </div>
      )}
    </div>
  );
